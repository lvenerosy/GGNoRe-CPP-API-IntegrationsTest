...
int main()
{
	...
	struct TestProgress
	{
		...
		const size_t StartTestIndex = 817; // Run the sln in development mode for optimal speed while keeping asserts, then if an assert is hit start from the failing test and run in debug mode
	};
